...
import type { Tab } from "@/model/navigation";
...
function handleTabClick(tabsPane: TabsPaneContext) {
  let route = tabs.value.find((e: any) => e.path === tabsPane.props.name);
  useRouterStore.setActivedMainSidebarMenuItem(route.path);
  router.push(route);
}
